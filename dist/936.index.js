...
/* harmony export */   "t": () => (/* binding */ __exportAll)
/* harmony export */ });
...
/* harmony export */   "default": () => (/* binding */ index_exports),
/* harmony export */   "doc": () => (/* binding */ doc_exports),
/* harmony export */   "format": () => (/* binding */ format2),
/* harmony export */   "formatWithCursor": () => (/* binding */ formatWithCursor2),
/* harmony export */   "getFileInfo": () => (/* binding */ get_file_info_default),
...
/* harmony export */ });
/* harmony import */ var _chunk_DrSxFLj_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(9209);
/* harmony import */ var path__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(1017);
/* harmony import */ var fs__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(7147);
/* harmony import */ var module__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(8188);
/* harmony import */ var url__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(7310);
...
/* harmony import */ var assert__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(9491);
/* harmony import */ var v8__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(4655);
/* harmony import */ var util__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(3837);
...
var doc_exports = /* @__PURE__ */ (0,_chunk_DrSxFLj_js__WEBPACK_IMPORTED_MODULE_0__.t)({
	builders: () => builders,
...
const require = (0,module__WEBPACK_IMPORTED_MODULE_3__.createRequire)(import.meta.url);
(0,path__WEBPACK_IMPORTED_MODULE_1__.dirname)((0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(import.meta.url));
...
var toPath = (urlOrPath) => isUrl(urlOrPath) ? url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath(urlOrPath) : urlOrPath;
var toAbsolutePath = (urlOrPath) => urlOrPath ? path__WEBPACK_IMPORTED_MODULE_1__.resolve(isUrl(urlOrPath) ? url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath(urlOrPath) : urlOrPath) : urlOrPath;
...
async function findInDirectory(nameOrNames, { typeCheck, cwd, allowSymlinks = true, filter: filter2 }) {
	const directory = toAbsolutePath(cwd) ?? process__WEBPACK_IMPORTED_MODULE_6__.cwd();
	const names = Array.isArray(nameOrNames) ? nameOrNames : [nameOrNames];
	for (const name of names) {
		const fileOrDirectory = path__WEBPACK_IMPORTED_MODULE_1__.join(directory, name);
...
async function safeStat(path15, allowSymlinks = true) {
	try {
		return await (allowSymlinks ? fs_promises__WEBPACK_IMPORTED_MODULE_5__.stat : fs_promises__WEBPACK_IMPORTED_MODULE_5__.lstat)(path15);
	} catch {}
}
...
function* iterateDirectoryUp(from, to) {
	let directory = toAbsolutePath(from) ?? process__WEBPACK_IMPORTED_MODULE_6__.cwd();
	let stopDirectory = toAbsolutePath(to);
	if (stopDirectory) {
		const relation = path__WEBPACK_IMPORTED_MODULE_1__.relative(stopDirectory, directory);
		if (relation[0] === "." || relation === directory) return;
	}
	stopDirectory = stopDirectory ? directory.slice(0, stopDirectory.length) : path__WEBPACK_IMPORTED_MODULE_1__.parse(directory).root;
	while (directory !== stopDirectory) {
		yield directory;
		directory = path__WEBPACK_IMPORTED_MODULE_1__.dirname(directory);
	}
	yield stopDirectory;
}
...
async function findProjectRoot(startDirectory, options8) {
	...
	return directory ? path__WEBPACK_IMPORTED_MODULE_1__.dirname(directory) : void 0;
}
...
async function loadEditorconfigInternal(file, { shouldCache }) {
	const root2 = await findProjectRoot(path__WEBPACK_IMPORTED_MODULE_1__.dirname(file), { shouldCache });
...
function loadEditorconfig(file, { shouldCache }) {
	file = path__WEBPACK_IMPORTED_MODULE_1__.resolve(file);
...
async function readFile(file) {
	...
	try {
		return await fs_promises__WEBPACK_IMPORTED_MODULE_5__.readFile(file, "utf8");
	} catch (error) {
...
async function importModuleDefault(file) {
	return (await __webpack_require__(9802)((0,url__WEBPACK_IMPORTED_MODULE_4__.pathToFileURL)(file).href)).default;
}
...
async function loadYaml(file) {
	const content = await read_file_default(file);
	if (!parseYaml) ({__parsePrettierYamlConfig: parseYaml} = await __webpack_require__.e(/* import() */ 824).then(__webpack_require__.bind(__webpack_require__, 9824)));
	try {
...
codes.ERR_INVALID_ARG_TYPE = createError(
	...
	*/
	(name, expected, actual) => {
		assert__WEBPACK_IMPORTED_MODULE_7__.ok(typeof name === "string", "'name' must be a string");
		...
		for (const value of expected) {
			assert__WEBPACK_IMPORTED_MODULE_7__.ok(typeof value === "string", "All expected entries have to be of type string");
			if (kTypes.has(value)) types.push(value.toLowerCase());
			else if (classRegExp.exec(value) === null) {
				assert__WEBPACK_IMPORTED_MODULE_7__.ok(value !== "object", "The value \"object\" should be written as \"Object\"");
				other.push(value);
...
codes.ERR_INVALID_PACKAGE_TARGET = createError(
	...
	*/
	(packagePath, key2, target, isImport = false, base = void 0) => {
		...
		if (key2 === ".") {
			assert__WEBPACK_IMPORTED_MODULE_7__.ok(isImport === false);
...
codes.ERR_INVALID_ARG_VALUE = createError(
	...
	*/
	(name, value, reason = "is invalid") => {
		let inspected = (0,util__WEBPACK_IMPORTED_MODULE_9__.inspect)(value);
...
function isErrorStackTraceLimitWritable() {
	try {
		if (v8__WEBPACK_IMPORTED_MODULE_8__.startupSnapshot.isBuildingSnapshot()) return false;
	} catch {}
...
function getMessage(key2, parameters, self) {
	...
	assert__WEBPACK_IMPORTED_MODULE_7__.ok(message !== void 0, "expected `message` to be found");
	if (typeof message === "function") {
		assert__WEBPACK_IMPORTED_MODULE_7__.ok(message.length <= parameters.length, `Code: ${key2}; The provided arguments length (${parameters.length}) does not match the required ones (${message.length}).`);
		return Reflect.apply(message, self, parameters);
	}
	const regex = /%[dfijoOs]/g;
	let expectedLength = 0;
	while (regex.exec(message) !== null) expectedLength++;
	assert__WEBPACK_IMPORTED_MODULE_7__.ok(expectedLength === parameters.length, `Code: ${key2}; The provided arguments length (${parameters.length}) does not match the required ones (${expectedLength}).`);
	if (parameters.length === 0) return message;
	parameters.unshift(message);
	return Reflect.apply(util__WEBPACK_IMPORTED_MODULE_9__.format, null, parameters);
}
function determineSpecificType(value) {
	...
	if (typeof value === "object") {
		...
		return `${(0,util__WEBPACK_IMPORTED_MODULE_9__.inspect)(value, { depth: -1 })}`;
	}
	let inspected = (0,util__WEBPACK_IMPORTED_MODULE_9__.inspect)(value, { colors: false });
...
function read2(jsonPath, { base, specifier }) {
	...
	let string;
	try {
		string = fs__WEBPACK_IMPORTED_MODULE_2__.readFileSync(path__WEBPACK_IMPORTED_MODULE_1__.toNamespacedPath(jsonPath), "utf8");
	} catch (error) {
	...
	if (string !== void 0) {
		...
		try {
			...
			const cause = error_;
			const error = new ERR_INVALID_PACKAGE_CONFIG(jsonPath, (base ? `"${specifier}" from ` : "") + (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base || specifier), cause.message);
			error.cause = cause;
			throw error;
		}
		result.exists = true;
...
function getPackageScopeConfig(resolved) {
	...
	while (true) {
		...
		const packageConfig = read2((0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(packageJSONUrl), { specifier: resolved });
	...
	return {
		pjsonPath: (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(packageJSONUrl),
		exists: false,
		type: "none"
	};
}
...
function getFileProtocolModuleFormat(url3, _context, ignoreErrors) {
	...
	throw new ERR_UNKNOWN_FILE_EXTENSION(value, (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(url3));
}
...
function emitInvalidSegmentDeprecation(target, request, match, packageJsonUrl, internal, base, isTarget) {
	if (process__WEBPACK_IMPORTED_MODULE_6__.noDeprecation) return;
	const pjsonPath = (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(packageJsonUrl);
	const double = doubleSlashRegEx.exec(isTarget ? target : request) !== null;
	process__WEBPACK_IMPORTED_MODULE_6__.emitWarning(`Use of deprecated ${double ? "double slash" : "leading or trailing slash matching"} resolving "${target}" for module request "${request}" ${request === match ? "" : `matched to "${match}" `}in the "${internal ? "imports" : "exports"}" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${(0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base)}` : ""}.`, "DeprecationWarning", "DEP0166");
}
function emitLegacyIndexDeprecation(url3, packageJsonUrl, base, main) {
	...
	const urlPath = (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(url3.href);
	const packagePath = (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(new URL(".", packageJsonUrl));
	const basePath = (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base);
	if (!main) process__WEBPACK_IMPORTED_MODULE_6__.emitWarning(`No "main" or "exports" field defined in the package.json for ${packagePath} resolving the main entry point "${urlPath.slice(packagePath.length)}", imported from ${basePath}.
Default "index" lookups for the main are deprecated for ES modules.`, "DeprecationWarning", "DEP0151");
	else if (path__WEBPACK_IMPORTED_MODULE_1__.resolve(packagePath, main) !== urlPath) process__WEBPACK_IMPORTED_MODULE_6__.emitWarning(`Package ${packagePath} has a "main" field set to "${main}", excluding the full filename and extension to the resolved file at "${urlPath.slice(packagePath.length)}", imported from ${basePath}.
 Automatic extension resolution of the "main" field is deprecated for ES modules.`, "DeprecationWarning", "DEP0151");
}
function tryStatSync(path15) {
	try {
		return (0,fs__WEBPACK_IMPORTED_MODULE_2__.statSync)(path15);
	} catch {}
}
function fileExists(url3) {
	const stats = (0,fs__WEBPACK_IMPORTED_MODULE_2__.statSync)(url3, { throwIfNoEntry: false });
...
function legacyMainResolve(packageJsonUrl, packageConfig, base) {
	...
	throw new ERR_MODULE_NOT_FOUND((0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(new URL(".", packageJsonUrl)), (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base));
}
function finalizeResolution(resolved, base, preserveSymlinks) {
	if (encodedSeparatorRegEx.exec(resolved.pathname) !== null) throw new ERR_INVALID_MODULE_SPECIFIER(resolved.pathname, "must not include encoded \"/\" or \"\\\" characters", (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base));
	let filePath;
	try {
		filePath = (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(resolved);
	} catch (error) {
		const cause = error;
	...
	if (stats && stats.isDirectory()) {
		const error = new ERR_UNSUPPORTED_DIR_IMPORT(filePath, (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base));
		error.url = String(resolved);
		throw error;
	}
	if (!stats || !stats.isFile()) {
		const error = new ERR_MODULE_NOT_FOUND(filePath || resolved.pathname, base && (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base), true);
		error.url = String(resolved);
		throw error;
	}
	if (!preserveSymlinks) {
		const real = (0,fs__WEBPACK_IMPORTED_MODULE_2__.realpathSync)(filePath);
		const { search, hash } = resolved;
		resolved = (0,url__WEBPACK_IMPORTED_MODULE_4__.pathToFileURL)(real + (filePath.endsWith(path__WEBPACK_IMPORTED_MODULE_1__.sep) ? "/" : ""));
...
function importNotDefined(specifier, packageJsonUrl, base) {
	return new ERR_PACKAGE_IMPORT_NOT_DEFINED(specifier, packageJsonUrl && (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(new URL(".", packageJsonUrl)), (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base));
}
function exportsNotFound(subpath, packageJsonUrl, base) {
	return new ERR_PACKAGE_PATH_NOT_EXPORTED((0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(new URL(".", packageJsonUrl)), subpath, base && (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base));
}
function throwInvalidSubpath(request, match, packageJsonUrl, internal, base) {
	throw new ERR_INVALID_MODULE_SPECIFIER(request, `request is not a valid match in pattern "${match}" for the "${internal ? "imports" : "exports"}" resolution of ${(0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(packageJsonUrl)}`, base && (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base));
}
function invalidPackageTarget(subpath, target, packageJsonUrl, internal, base) {
	target = typeof target === "object" && target !== null ? JSON.stringify(target, null, "") : `${target}`;
	return new ERR_INVALID_PACKAGE_TARGET((0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(new URL(".", packageJsonUrl)), subpath, target, internal, base && (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base));
}
...
function resolvePackageTarget(packageJsonUrl, target, subpath, packageSubpath, base, pattern, internal, isPathMap, conditions) {
	...
	if (typeof target === "object" && target !== null) {
		...
		while (++i < keys.length) {
			const key2 = keys[i];
			if (isArrayIndex(key2)) throw new ERR_INVALID_PACKAGE_CONFIG2((0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(packageJsonUrl), base, "\"exports\" cannot contain numeric property keys.");
		}
		i = -1;
		while (++i < keys.length) {
			const key2 = keys[i];
...
function isConditionalExportsMainSugar(exports, packageJsonUrl, base) {
	...
	while (++keyIndex < keys.length) {
		...
		else if (isConditionalSugar !== currentIsConditionalSugar) throw new ERR_INVALID_PACKAGE_CONFIG2((0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(packageJsonUrl), base, `"exports" cannot contain some keys starting with '.' and some not. The exports object must either be an object of package subpath keys or an object of main entry condition name keys only.`);
	}
	return isConditionalSugar;
}
function emitTrailingSlashPatternDeprecation(match, pjsonUrl, base) {
	if (process__WEBPACK_IMPORTED_MODULE_6__.noDeprecation) return;
	const pjsonPath = (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(pjsonUrl);
	if (emittedPackageWarnings.has(pjsonPath + "|" + match)) return;
	emittedPackageWarnings.add(pjsonPath + "|" + match);
	process__WEBPACK_IMPORTED_MODULE_6__.emitWarning(`Use of deprecated trailing slash pattern mapping "${match}" in the "exports" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${(0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base)}` : ""}. Mapping specifiers ending in "/" is no longer supported.`, "DeprecationWarning", "DEP0155");
}
...
function packageImportsResolve(name, base, conditions) {
	if (name === "#" || name.startsWith("#/") || name.endsWith("/")) throw new ERR_INVALID_MODULE_SPECIFIER(name, "is not a valid internal imports specifier name", (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base));
	let packageJsonUrl;
	const packageConfig = getPackageScopeConfig(base);
	if (packageConfig.exists) {
		packageJsonUrl = (0,url__WEBPACK_IMPORTED_MODULE_4__.pathToFileURL)(packageConfig.pjsonPath);
...
function parsePackageName(specifier, base) {
	...
	if (!validPackageName) throw new ERR_INVALID_MODULE_SPECIFIER(specifier, "is not a valid package name", (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base));
	return {
		packageName,
...
function packageResolve(specifier, base, conditions) {
	if (module__WEBPACK_IMPORTED_MODULE_3__.builtinModules.includes(specifier)) return new URL("node:" + specifier);
	const { packageName, packageSubpath, isScoped } = parsePackageName(specifier, base);
	const packageConfig = getPackageScopeConfig(base);
	if (packageConfig.exists) {
		const packageJsonUrl2 = (0,url__WEBPACK_IMPORTED_MODULE_4__.pathToFileURL)(packageConfig.pjsonPath);
	...
	let packageJsonPath = (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(packageJsonUrl);
	let lastPath;
	do {
		...
		if (!stat2 || !stat2.isDirectory()) {
			...
			packageJsonPath = (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(packageJsonUrl);
			continue;
		}
	...
	throw new ERR_MODULE_NOT_FOUND(packageName, (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(base), false);
}
...
function moduleResolve(specifier, base, conditions, preserveSymlinks) {
	...
	if (shouldBeTreatedAsRelativeOrAbsolutePath(specifier)) try {
		...
		if (isRemote && !module__WEBPACK_IMPORTED_MODULE_3__.builtinModules.includes(specifier)) {
			const error = new ERR_UNSUPPORTED_RESOLVE_REQUEST(specifier, base);
			error.cause = error_;
			throw error;
		}
	...
	assert__WEBPACK_IMPORTED_MODULE_7__.ok(resolved !== void 0, "expected to be defined");
...
function checkIfDisallowedImport(specifier, parsed, parsedParentURL) {
	if (parsedParentURL) {
		...
		if (parentProtocol === "http:" || parentProtocol === "https:") {
			...
			if (module__WEBPACK_IMPORTED_MODULE_3__.builtinModules.includes(specifier)) throw new ERR_NETWORK_IMPORT_DISALLOWED(specifier, parsedParentURL, "remote imports cannot import from a local location.");
...
function defaultResolve(specifier, context = {}) {
	...
	assert__WEBPACK_IMPORTED_MODULE_7__.ok(parentURL !== void 0, "expected `parentURL` to be defined");
...
function importFromFile(specifier, parent) {
	return __webpack_require__(9802)(resolve2(specifier, (0,url__WEBPACK_IMPORTED_MODULE_4__.pathToFileURL)(parent).href));
}
var import_from_file_default = importFromFile;
function requireFromFile(id, parent) {
	return (0,module__WEBPACK_IMPORTED_MODULE_3__.createRequire)(parent)(id);
}
...
async function loadConfig(configFile) {
	const { base: fileName, ext: extension } = path__WEBPACK_IMPORTED_MODULE_1__.parse(configFile);
...
function loadPrettierConfig(configFile, { shouldCache }) {
	configFile = path__WEBPACK_IMPORTED_MODULE_1__.resolve(configFile);
...
function getSearchFunction(stopDirectory) {
	stopDirectory = stopDirectory ? path__WEBPACK_IMPORTED_MODULE_1__.resolve(stopDirectory) : void 0;
...
function searchPrettierConfig(startDirectory, options8 = {}) {
	startDirectory = startDirectory ? path__WEBPACK_IMPORTED_MODULE_1__.resolve(startDirectory) : process.cwd();
...
async function loadPrettierConfig2(file, options8) {
	...
	if (!configFile) configFile = await searchPrettierConfig(file ? path__WEBPACK_IMPORTED_MODULE_1__.dirname(path__WEBPACK_IMPORTED_MODULE_1__.resolve(file)) : void 0, { shouldCache });
...
async function resolveConfig(fileUrlOrPath, options8) {
	...
	if (Array.isArray(merged.plugins)) merged.plugins = merged.plugins.map((value) => typeof value === "string" && value.startsWith(".") ? path__WEBPACK_IMPORTED_MODULE_1__.resolve(path__WEBPACK_IMPORTED_MODULE_1__.dirname(result.configFile), value) : value);
	return merged;
}
async function resolveConfigFile(fileUrlOrPath) {
	return await searchPrettierConfig(fileUrlOrPath ? path__WEBPACK_IMPORTED_MODULE_1__.dirname(path__WEBPACK_IMPORTED_MODULE_1__.resolve(toPath(fileUrlOrPath))) : void 0, { shouldCache: false }) ?? null;
}
function mergeOverrides(configResult, filePath) {
	const { config, configFile } = configResult || {};
	const { overrides, ...options8 } = config || {};
	if (filePath && overrides) {
		const relativeFilePath = path__WEBPACK_IMPORTED_MODULE_1__.relative(path__WEBPACK_IMPORTED_MODULE_1__.dirname(configFile), filePath);
...
function breakTies(tiesToBreak, options8) {
	...
	for (indexOfFirstLeadingComment = tieCount; indexOfFirstLeadingComment > 0; --indexOfFirstLeadingComment) {
		...
		(0,assert__WEBPACK_IMPORTED_MODULE_7__.strictEqual)(currentCommentPrecedingNode, precedingNode);
		(0,assert__WEBPACK_IMPORTED_MODULE_7__.strictEqual)(currentCommentFollowingNode, followingNode);
...
var getFileBasename = (file) => {
	try {
		return path__WEBPACK_IMPORTED_MODULE_1__.basename(toPath(file));
	} catch {
		return "";
	}
};
...
function getLanguageByIsSupported(languages2, file) {
	if (!file) return;
	if (isUrl(file)) try {
		file = (0,url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath)(file);
	} catch {
		return;
	}
...
function calculateRange(text, opts, ast) {
	...
	(0,assert__WEBPACK_IMPORTED_MODULE_7__.ok)(end > start);
...
var estreePlugin = createParsersAndPrinters([{
	importPlugin: () => __webpack_require__.e(/* import() */ 260).then(__webpack_require__.bind(__webpack_require__, 5260)),
...
var { parsers, printers } = createParsersAndPrinters([
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 823).then(__webpack_require__.bind(__webpack_require__, 8823)),
		parsers: ["acorn", "espree"]
	},
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 726).then(__webpack_require__.bind(__webpack_require__, 2726)),
		parsers: [
			"__ng_action",
			"__ng_binding",
			"__ng_interpolation",
	...
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 777).then(__webpack_require__.bind(__webpack_require__, 3777)),
		parsers: [
			"babel",
			"babel-flow",
			"babel-ts",
	...
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 652).then(__webpack_require__.bind(__webpack_require__, 5652)),
		parsers: ["flow"]
	},
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 336).then(__webpack_require__.bind(__webpack_require__, 4336)),
		parsers: ["glimmer"],
		printers: ["glimmer"]
	},
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 856).then(__webpack_require__.bind(__webpack_require__, 5856)),
		parsers: ["graphql"],
		printers: ["graphql"]
	},
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 365).then(__webpack_require__.bind(__webpack_require__, 5365)),
		parsers: [
			"html",
			"angular",
			"vue",
	...
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 408).then(__webpack_require__.bind(__webpack_require__, 3408)),
		parsers: [
			"markdown",
			"mdx",
			"remark"
	...
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 31).then(__webpack_require__.bind(__webpack_require__, 9031)),
		parsers: ["meriyah"]
	},
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 117).then(__webpack_require__.bind(__webpack_require__, 6117)),
		parsers: [
			"css",
			"less",
			"scss"
	...
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 814).then(__webpack_require__.bind(__webpack_require__, 3814)),
		parsers: ["typescript"]
	},
	{
		importPlugin: () => __webpack_require__.e(/* import() */ 824).then(__webpack_require__.bind(__webpack_require__, 9824)),
		parsers: ["yaml"],
		printers: ["yaml"]
	}
]);
...
function importFromDirectory(specifier, directory) {
	return import_from_file_default(specifier, path__WEBPACK_IMPORTED_MODULE_1__.join(directory, "noop.js"));
}
var import_from_directory_default = importFromDirectory;
async function importPlugin(name, cwd) {
	if (isUrl(name)) return __webpack_require__(9802)(name);
	if (path__WEBPACK_IMPORTED_MODULE_1__.isAbsolute(name)) return __webpack_require__(9802)((0,url__WEBPACK_IMPORTED_MODULE_4__.pathToFileURL)(name).href);
	try {
		return await __webpack_require__(9802)((0,url__WEBPACK_IMPORTED_MODULE_4__.pathToFileURL)(path__WEBPACK_IMPORTED_MODULE_1__.resolve(name)).href);
	} catch {
...
var slash = path__WEBPACK_IMPORTED_MODULE_1__.sep === "\\" ? (filePath) => method_replace_all_default(0, filePath, "\\", "/") : (filePath) => filePath;
function getRelativePath(file, ignoreFile) {
	const ignoreFilePath = toPath(ignoreFile);
	const filePath = isUrl(file) ? url__WEBPACK_IMPORTED_MODULE_4__.fileURLToPath(file) : path__WEBPACK_IMPORTED_MODULE_1__.resolve(file);
	return path__WEBPACK_IMPORTED_MODULE_1__.relative(ignoreFilePath ? path__WEBPACK_IMPORTED_MODULE_1__.dirname(ignoreFilePath) : process.cwd(), filePath);
}
